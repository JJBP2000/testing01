  ...
  saveReview() {
    if (!this.message || !this.rating) {
      Swal.fire({
        icon: 'error',
        title: 'Error!',
        text: 'LA CALIFICACIÓN Y EL MENSAJE SON OBLIGATORIOS',
      });
      return;
    }
    let data = {
    ...
    const handleResponse = (response: any, successMessage: string) => {
      ...
      if (INDEX != -1) {
        this.sale_details[INDEX].review = response.review;
      }
    };
  
    if (this.sale_detail_selected.review) {
      this.tiendaAuth.updateReview(data, this.sale_detail_selected.review.id).subscribe(resp => {
        handleResponse(resp, 'LA RESEÑA SE EDITO CORRECTAMENTE');
      });
    } else {
      this.tiendaAuth.registerReview(data).subscribe(resp => {
        handleResponse(resp, 'LA RESEÑA SE REGISTRO CORRECTAMENTE');
      });
    }
  }
